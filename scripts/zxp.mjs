...
const distPluginDir = path.join(root, "dist", "rdzTools");
const releaseDir = path.join(root, "release");

const signerPath = process.env.ZXP_SIGN_CMD || "/Users/ryder/Desktop/zxp-sign/ZXPSignCmd";
const certPath = process.env.ZXP_CERT || path.join(root, "certs", "rdzTools.p12");
...
async function main() {
  ...
  const outputPath = path.join(releaseDir, `rdzTools-${version}.zxp`);
