var rdzTools = (function () {
  ...
  function removeOldTextAnimators(layer) {
    removeTextAnimatorsByMode(layer, "all");
  }

  function removeTextAnimatorsByMode(layer, mode) {
    ...
    for (var animatorIndex = animators.numProperties; animatorIndex >= 1; animatorIndex -= 1) {
      ...
      var isTextAnimator = name === "AW_Animator" || name.indexOf("TA_Word_") === 0 || name.indexOf("TA_") === 0;
      var isOutAnimator = name.indexOf("_Out_") !== -1;
      if (isTextAnimator && (mode === "all" || (mode === "in" && !isOutAnimator) || (mode === "out" && isOutAnimator))) {
  ...
  function getTextLayerFillColor(layer) {
    try {
      var textDocument = layer.property("ADBE Text Properties").property("ADBE Text Document").value;
      if (textDocument && textDocument.fillColor && textDocument.fillColor.length >= 3) {
        return [textDocument.fillColor[0], textDocument.fillColor[1], textDocument.fillColor[2]];
      }
  ...
  function applyTextStartColor(animProps, t0, t1, settings, baseColor, easeSetter, colorOut) {
    if (!settings.startColorEnabled) {
      return;
    }

    var colorProp = addAnimatorProperty(animProps, "ADBE Text Fill Color", "Fill Color");
    if (!colorProp) {
      return;
    }

    try {
      key2(colorProp, t0, colorOut ? baseColor : settings.startColor, t1, colorOut ? settings.startColor : baseColor, easeSetter || setWordEase);
    } catch (colorError) {}
  }
  ...
  function gateSelectorUntilTime(rangeSelector, time, comp) {
    try {
      var advanced = rangeSelector.property(7);
      if (!advanced) {
        return;
      ...
      var amount = advanced.property(4);
      if (!amount) {
        return;
      }

      var preTime = time - Math.max(0.001, comp.frameDuration / 10);
      amount.setValueAtTime(preTime, 0);
      amount.setValueAtTime(time, 100);
      var preKey = getKeyIndexAtTime(amount, preTime);
      var startKey = getKeyIndexAtTime(amount, time);
      if (preKey) {
        amount.setInterpolationTypeAtKey(preKey, KeyframeInterpolationType.HOLD, KeyframeInterpolationType.HOLD);
      }
      if (startKey) {
        amount.setInterpolationTypeAtKey(startKey, KeyframeInterpolationType.HOLD, KeyframeInterpolationType.HOLD);
      }
    } catch (gateError) {}
  }
  ...
  function normalizeWordSettings(comp, payload) {
    return {
      ...
      startColorEnabled: payload.startColorEnabled === true,
      startColor: parseHexColor(payload.startColor, [0.176, 0.549, 1]),
  ...
  function applyWordAnimation(layer, comp, settings, options) {
    removeTextAnimatorsByMode(layer, "in");
    ...
    var baseColor = getTextLayerFillColor(layer);
    ...
    for (var i = 0; i < numWords; i += 1) {
      ...
      applyTextStartColor(animProps, t0, t1, settings, baseColor, setWordEase);

      if (posProp && (startX !== 0 || startY !== 0)) {
        if (options.positionBounce) {
          var settleTime = t0 + (comp.frameDuration * 10);
          try {
            posProp.setValueAtTime(t0, [startX, startY, 0]);
            posProp.setValueAtTime(settleTime, [0, 0, 0]);
          } catch (positionBounce3dError) {
            posProp.setValueAtTime(t0, [startX, startY]);
            posProp.setValueAtTime(settleTime, [0, 0]);
          }
          for (var posKeyIndex = Math.max(1, posProp.numKeys - 1); posKeyIndex <= posProp.numKeys; posKeyIndex += 1) {
            try {
              posProp.setInterpolationTypeAtKey(posKeyIndex, KeyframeInterpolationType.LINEAR, KeyframeInterpolationType.LINEAR);
            } catch (positionLinearError) {}
          }
          try {
            ensureBounceControls(layer, normalizeBounceSettings({}));
            posProp.expression = buildBounceExpression();
            posProp.expressionEnabled = true;
          } catch (positionBounceExpressionError) {}
        } else {
          try {
  ...
  function applyCharacterAnimation(layer, comp, settings, options) {
    removeTextAnimatorsByMode(layer, "in");
  ...
  function applyCharacterBounceAnimation(layer, comp, settings) {
    ...
    for (var i = 0; i < totalChars; i += 1) {
      ...
      applyTextStartColor(animProps, t0, t3, settings, baseColor, setWordEase);
  ...
  function applyWordRotateBounceAnimation(layer, comp, settings) {
    ...
    for (var i = 0; i < numWords; i += 1) {
      ...
      var t3 = t0 + settings.wordDur;
      applyTextStartColor(animProps, t0, t3, settings, baseColor, setSnappyEase);
  ...
  function applyWordOutAnimation(layer, comp, settings, options) {
    removeTextAnimatorsByMode(layer, "out");
    configureTextAnimatorWordGrouping(layer);
    ...
    for (var i = 0; i < numWords; i += 1) {
      ...
      var staggerIndex = options.reverseStagger ? (numWords - 1 - i) : i;
      var t0 = settings.startSec + (staggerIndex * settings.stagger);
      var t1 = t0 + settings.wordDur;
      gateSelectorUntilTime(rangeSelector, t0, comp);
      applyTextStartColor(animProps, t0, t1, settings, baseColor, setWordEase, true);

      if (posProp && (endX !== 0 || endY !== 0)) {
        try {
          key2(posProp, t0, [0, 0, 0], t1, [endX, endY, 0], setWordEase);
        } catch (position3dError) {
          key2(posProp, t0, [0, 0], t1, [endX, endY], setWordEase);
        }
      }
      if (opacityProp) {
        key2(opacityProp, t0, 100, t1, 0, setWordEase);
      }
      if (settings.blurEnabled && blurProp) {
        try {
          key2(blurProp, t0, [0, 0], t1, [settings.blurAmt, settings.blurAmt], setWordEase);
        } catch (blur2dError) {
          try { key2(blurProp, t0, 0, t1, settings.blurAmt, setWordEase); } catch (blur1dError) {}
        }
      }
      if (scaleProp) {
        try {
          key2(scaleProp, t0, [100, 100, 100], t1, [options.scaleEnd, options.scaleEnd, 100], setWordEase);
        } catch (scale3dError) {
          key2(scaleProp, t0, [100, 100], t1, [options.scaleEnd, options.scaleEnd], setWordEase);
        }
      }
      if (rotationProp) {
        key2(rotationProp, t0, 0, t1, options.rotationEnd, setWordEase);
      }
    }
  ...
  function applyCharacterOutAnimation(layer, comp, settings, options) {
    ...
    for (var i = 0; i < totalChars; i += 1) {
      ...
      applyTextStartColor(animProps, t0, t1, settings, baseColor, setSnappyEase, true);
  ...
  function normalizeLayerSettings(comp, payload) {
    return {
      ...
      direction: payload.direction || "Bottom",
  ...
  function getLayerDirectionOffset(settings) {
    var direction = String(settings.direction || "Bottom").toLowerCase();
    if (direction === "top") {
      return { offsetX: 0, offsetY: -settings.distance };
    }
    if (direction === "right") {
      return { offsetX: settings.distance, offsetY: 0 };
    }
    if (direction === "left") {
      return { offsetX: -settings.distance, offsetY: 0 };
    }
  ...
  function applyLayerEntrance(layer, comp, settings, options) {
    ...
    applyLayerStartColor(layer, t0, t1, settings);
  ...
  function getEffectProperty(effect, ids) {
    if (!effect) {
      return null;
    }
  ...
  function applyLayerStartColor(layer, t0, t1, settings) {
    if (!settings.startColorEnabled) {
      removeEffectsByName(layer, "rdzTools Start Color");
      removeEffectsByName(layer, "rdzTools Animation Color");
      return;
    }
    ...
    var fill = addOrGetEffectFlexible(layer, "ADBE Fill", "Fill");
    fill.name = "rdzTools Animation Color";
    setEffectValue(fill, [3, "Color"], settings.startColor);

    var opacityProp = getEffectProperty(fill, [7, "Opacity"]);
    if (!opacityProp) {
      return;
    }
  ...
  function removeEffectsByName(layer, effectName) {
    var effects = layer.property("ADBE Effect Parade");
    if (!effects) {
      return 0;
    }
  ...
  function setVectorStrokeValues(stroke, settings) {
    ...
    setEffectValue(stroke, ["ADBE Vector Stroke Color", "Color", 2], settings.color || [1, 1, 1, 1]);
    setEffectValue(stroke, ["ADBE Vector Stroke Opacity", "Opacity", 4], settings.opacity || 65);
  ...
  function applyLayerScalePop(layer, comp, settings) {
    ...
    var t1 = t0 + frame;
    var t2 = t1 + frame;
    applyLayerStartColor(layer, opacityT0, t2, settings);
  ...
  function primeCharacterBodies(bodies, settings, comp) {
    ...
    for (var b = 0; b < bodies.length; b += 1) {
      ...
      if (Math.abs(dx) + Math.abs(dy) > 0.001) {
        ...
        var vectorY = (dy / outwardLength) * 0.35 + Math.sin(randomAngle) * randomWeight - (0.72 + seededUnitValue(seed + 223) * 0.18);
        var vectorLength = Math.max(0.001, Math.sqrt(vectorX * vectorX + vectorY * vectorY));
  ...
  function applyTool(toolId, rawPayload) {
    ...
    try {
      ...
      if (toolId === "wordIn" || toolId === "charIn" || toolId === "wordBounceIn" || toolId === "wordBlurRight" || toolId === "wordBlurLeft" || toolId === "wordBlurUp" || toolId === "wordBlurDown" || toolId === "wordRotateIn" || toolId === "charBounceIn" || toolId === "wordOut" || toolId === "wordBlurOutRight" || toolId === "wordBlurOutLeft" || toolId === "wordBlurOutUp" || toolId === "wordBlurOutDown" || toolId === "wordRotateOut" || toolId === "charShrinkOut" || toolId === "charScatterOut") {
        var textLayer = getTextLayer(comp);
        if (!textLayer) {
          throw new Error("No text layer found in comp.");
        }
        ...
        if (toolId === "wordIn" || toolId === "charIn" || toolId === "wordBounceIn") {
          var wordInDirection = String(wordSettings.direction || "Right").toLowerCase();
          if (wordInDirection === "left") {
            options = { offsetX: -wordSettings.distance, offsetY: 0 };
          } else if (wordInDirection === "bottom") {
            options = { offsetX: 0, offsetY: wordSettings.distance };
          } else if (wordInDirection === "top") {
            options = { offsetX: 0, offsetY: -wordSettings.distance };
          } else {
            options = { offsetX: wordSettings.distance, offsetY: 0 };
          }
          if (toolId === "wordBounceIn") {
            options.positionBounce = true;
          }
          if (toolId === "charIn") {
            applyCharacterAnimation(textLayer, comp, wordSettings, options);
            return "OK: Applied " + toolId + " to " + textLayer.name + ".";
          }
        }
        ...
        if (toolId === "wordOut") {
          var wordOutDirection = String(wordSettings.direction || "Right").toLowerCase();
          if (wordOutDirection === "left") {
            options = { offsetX: -wordSettings.distance, offsetY: 0 };
          } else if (wordOutDirection === "up") {
            options = { offsetX: 0, offsetY: -wordSettings.distance };
          } else if (wordOutDirection === "down") {
            options = { offsetX: 0, offsetY: wordSettings.distance };
          } else {
            options = { offsetX: wordSettings.distance, offsetY: 0 };
          }
          options.reverseStagger = wordOutDirection === "right";
        }
        ...
        if (toolId === "wordOut" || toolId === "wordBlurOutRight" || toolId === "wordBlurOutLeft" || toolId === "wordBlurOutUp" || toolId === "wordBlurOutDown") {
      ...
      if (toolId === "layerFadeUp" || toolId === "layerBounceUp" || toolId === "layerSlideRight" || toolId === "layerSlideLeft" || toolId === "layerRotatePop") {
        ...
        for (var m = 0; m < motionLayers.length; m += 1) {
          if (toolId === "layerFadeUp") {
            applyLayerEntrance(motionLayers[m], comp, layerSettings, getLayerDirectionOffset(layerSettings));
